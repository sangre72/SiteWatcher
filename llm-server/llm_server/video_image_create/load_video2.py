...
# 비디오 로드 및 프레임 추출 함수
def load_video_and_detect_edges(video_path):
...
# 비디오 로드 및 엣지 검출
video_edge_vectors = load_video_and_detect_edges(video_path)
...
# 시간 축을 추가하여 시각화
time_labels = np.arange(len(edge_vectors_3d))
for i in range(len(edge_vectors_3d)):
    ax.text(edge_vectors_3d[i, 0], edge_vectors_3d[i, 1], edge_vectors_3d[i, 2], '%d' % time_labels[i], size=5, zorder=1, color='k')
