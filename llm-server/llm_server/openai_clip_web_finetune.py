...
from openai_clip_web import ImageVector
...
class ImageSearchAppFineTune:
    def __init__(self):
        self.session = Session()
        self.model = CLIPModel.from_pretrained("output/clip-vit-base-patch32-finetuned", use_auth_token=hf_token)
        self.processor = CLIPProcessor.from_pretrained("output/clip-vit-base-patch32-finetuned", use_auth_token=hf_token)
    ...
    def searchImages(self, query):
        try:
            ...
            for result in results:
                # 이미지 벡터와 텍스트 벡터 간의 유사도 계산
                image_vector = result.vector
                text_vector = result.text_vector
                image_similarity = self.cosine_similarity(text_emb, image_vector) if image_vector else 0
                text_similarity = self.cosine_similarity(text_emb, text_vector) if text_vector else 0

                # 더 높은 유사도를 기준으로 유사도 결정
                similarity = max(image_similarity, text_similarity * 2)
