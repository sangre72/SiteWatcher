...
def seed():
    ...
    # Meta-Llama 모델과 토크나이저 로드
    model_id = "sentence-transformers/all-MiniLM-L6-v2" #"meta-llama/Meta-Llama-3-8B-Instruct"
    try:
        tokenizer = AutoTokenizer.from_pretrained(model_id, cache_dir=cache_dir, use_auth_token=hf_token)
        tokenizer.add_special_tokens({'pad_token': '[PAD]'})
        model = AutoModel.from_pretrained(model_id, cache_dir=cache_dir, use_auth_token=hf_token)
