...
interface ChartProps {
    id: string;
    onMouseMove: (event: any, chartId: string) => void;
}
...
interface SeriesData {
    name: string;
    data: { x: number; y: number; fillColor: string }[];
}
const width = 200;
...
const NodeCPUUsageChart: React.FC<ChartProps> = ({ id, onMouseMove }) => {
    ...
    const [chart2Options, setChart2Options] = useState<ApexOptions>({
        ...
        chart: {
            ...
            events: {
                mouseMove: function(event, chartContext, config) {
                    onMouseMove(config, id);
                }
            }
        },
        stroke: {
            colors: ['#FF4560','#cFcFEF'],
            width: 1, // 라인의 굵기를 1px로 설정
            curve: 'smooth',
        },
        markers: {
            size: 1,
        ...
        xaxis: {
            ...
            min: startTime,
            max: endTime,
            tickAmount: 3, // 시작과 종료 포인트만 표기하기 위해 2로 설정
            labels: {
    ...
    return (
        <div ref={chartContainerRef} style={{marginTop: '25px'}}>
            <ApexCharts options={chart2Options} series={series} type="line"  />
        </div>
    );
};
