...
const ImageSearch: React.FC = () => {
  ...
  const searchImages = async () => {
    try {
      const response = await axios.post('http://localhost:9999/ai/image_search', { query });
      setResults(response.data.matches);
      setOthers(response.data.others);
      setDescriptions(new Array(response.data.matches.length + response.data.others.length).fill(''));
      addBlinkingClass();
    } catch (error) {
