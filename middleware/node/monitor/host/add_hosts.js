...
module.exports = { addOrUpdateHostInfo };

async function addOrUpdateHostInfo(req, res) {
    ...
    try {
        ...
        hosts.forEach(host => {
            console.log(host);
            const newHostLine = `${host.hostname}|${host.ip_addr}|${host.ssh_port}|${host.jmx_port}|${host.was_directory}|${host.was_start_command}|${host.was_pid}|${host.database_port}|${host.username}|${host.password}`;
            hostMap.set(host.hostname, newHostLine.replaceAll('\n',''));
        });
